    ...
    def test_serves_graph_and_persists_feedback(self) -> None:
        with TemporaryDirectory() as directory:
            ...
            try:
                ...
                self.assertIn('type: "neighborhood", view: "flow", label: "Module neighborhood"', diagrams_script)
                self.assertIn("function activateProjection", diagrams_script)
                self.assertIn("function validateProjectionChoice", diagrams_script)
                self.assertIn("projectionForm.addEventListener", diagrams_script)
                self.assertIn("function setProjectionDepth", diagrams_script)
                self.assertIn("const depth = Number(depthSelect.value)", diagrams_script)
                self.assertIn("projection.depth", diagrams_script)
