    ...
    def test_serves_graph_and_persists_feedback(self) -> None:
        with TemporaryDirectory() as directory:
            ...
            try:
                ...
                self.assertEqual(index.count("data-inspector-tab="), 1)
                self.assertIn('data-inspector-tab="chat"', index)
                ...
                self.assertIn('<option value="simple">Simple</option>', index)
                self.assertIn('<option value="spec">Spec only</option>', index)
                ...
                self.assertNotIn('data-chat-mode=', index)
                self.assertIn('id="explore-context"', index)
                self.assertNotIn('id="explore-agent-mode"', index)
                self.assertIn("Ask about this code", index)
                ...
                self.assertNotIn('src="/handoff.js"', index)
                ...
                self.assertLess(draft_group, index.index('id="reset-design"'))
                ...
                self.assertIn('"node.toggle-expansion", "node.expand", "node.collapse", "view.toggle-context", "node.hide"', commands_script)
                self.assertIn("focusRenderedGraphNode()", commands_script)
                self.assertIn('arrowright: "node.expand"', commands_script)
                self.assertIn('arrowleft: "node.collapse"', commands_script)
                self.assertIn('c: "view.toggle-context"', commands_script)
                self.assertIn('h: "node.hide"', commands_script)
                self.assertIn('data-command="node.hide"', index)
                self.assertIn('id="context-visibility"', index)
                ...
                self.assertIn("if (state.callTrace) graph = callTraceGraph();", app_script)
                self.assertIn("else if (state.graphProjection) graph = state.graphProjection.graph;", app_script)
                self.assertIn("graphViews.highlightedGraph(graph, state.selected)", app_script)
                ...
                self.assertIn("else graph = flowJourneyGraph();", app_script)
                ...
                self.assertIn("else if (state.layoutLocked && state.layoutSnapshot) graph = state.layoutSnapshot.graph", app_script)
                ...
                self.assertIn("function highlightedGraph", graph_views_script)
                ...
                self.assertIn(".chat-panel { display: grid; grid-template-rows: auto minmax(0, 1fr) auto", styles)
                self.assertIn("grid-template-columns: minmax(0, 1fr) minmax(0, 1fr) minmax(76px, 1.35fr)", styles)
                self.assertIn(".chat-form > .chat-actions { min-width: 0; display: grid", styles)
                self.assertIn(".chat-form .submit-button { min-width: 0; margin-left: 0", styles)
